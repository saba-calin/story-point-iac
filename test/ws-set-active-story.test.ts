...
import { handler } from "../lambda/ws-set-active-story/index";

describe("ws-set-active-story", () => {
  ...
  it("sends error when room not found", async () => {
    ...
    const result = await handler(makeEvent({ action: "setActiveStory", roomId: "room1", storyId: "s1" }));
  ...
  it("sends error when user is not owner", async () => {
    ...
    const result = await handler(makeEvent({ action: "setActiveStory", roomId: "room1", storyId: "s1" }, "notowner"));
    expect(result.statusCode).toBe(200);
    expect(mockApiGwSend).toHaveBeenCalledTimes(2);
  });
  ...
  it("returns ok without broadcast when story is already active", async () => {
    mockDynamoSend.mockResolvedValueOnce({
      Item: { roomId: "room1", status: "OPEN", ownerUsername: "owner" }
    });
    mockDynamoSend.mockResolvedValueOnce({
      Item: { roomId: "room1", storyId: "s1", status: "ACTIVE" }
    });
    const result = await handler(makeEvent({ action: "setActiveStory", roomId: "room1", storyId: "s1" }));
    expect(result.statusCode).toBe(200);
  ...
  it("sets story active and broadcasts to connections", async () => {
    mockDynamoSend.mockResolvedValueOnce({
      Item: { roomId: "room1", status: "OPEN", ownerUsername: "owner" }
    });
    mockDynamoSend.mockResolvedValueOnce({
      Item: { roomId: "room1", storyId: "s1", status: "NON_ACTIVE" }
    });
    mockDynamoSend.mockResolvedValueOnce({ Items: [] });
    mockDynamoSend.mockResolvedValueOnce({ Items: [] });
    mockDynamoSend.mockResolvedValueOnce({});
    mockDynamoSend.mockResolvedValueOnce({
      Items: [{ connectionId: "conn1", username: "owner" }]
    });
